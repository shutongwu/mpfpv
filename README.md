...
面向恶劣网络环境的多路径冗余组网工具。通过服务器中继，整合 UDP 多路径冗余和 TUN 虚拟 IP 组网，单二进制、纯用户态、零依赖。

适用于 4G/5G 蜂窝网络、卫星链路、多 WAN 等高丢包、高抖动场景。

> **安全声明**：本项目**不提供任何加密或安全保护**。`teamKey` 仅用于配对校验（防误连），所有流量均为明文传输。如需安全通信，请在上层叠加 WireGuard 等加密隧道。**请勿在不可信网络中传输敏感数据。**
