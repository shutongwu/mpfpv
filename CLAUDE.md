...
make client-arm32         # build/client/arm/mpfpv-linux-arm (ARMv7)
...
make test-integration                            # 同上
go test -run TestDedup ./internal/protocol/      # 单个测试

# 清理
make clean                # 删除 build/ 和本地二进制
```

版本号通过 git tags 自动注入（`-X main.version`）。
...
- Commit message 使用中文
- 标准格式：`feat:` / `fix:` / `refactor:` / `test:` / `docs:` + 中文描述
